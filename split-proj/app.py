                    ...
                    for trans in transactions:
                        if isinstance(trans, dict):
                            ...
                            if payment_mode == 'BANCNET' and original_line:
                                ...
                                amount_match = re.search(r'(\d{6})\d*$', original_line)
                                if amount_match:
                                    amount_str = amount_match.group(1)
                                    amount = float(amount_str) / 100
                                    group_total += amount
                                ...
                                date_match = re.search(r'\d{14}(\d{6})', original_line)
                                if date_match:
                                    date_str = date_match.group(1)
                                    formatted_date = f"{date_str[2:4]}/{date_str[:2]}/2025"
                                    dates.add(f"{formatted_date}")
            ...
            if payment_mode == 'METROBANK':
                ...
                for line in data.get('raw_contents', []):
                    ...
                    if payment_mode == 'PNB':
                        ...
                        # For BANCNET, amount is at index 1
                        if len(fields) > 1:
                            try:
                                amount_str = fields[1].replace(',', '')
                                amount = float(amount_str)
                                backend_total += amount
                            except (ValueError, TypeError) as e:
                                logger.error(f"Error converting BANCNET amount: {fields[1]}, error: {str(e)}")
                                amount = 0
        ...
        if payment_mode == 'CIS':
            ...
            for line in lines:
                ...
                if asterisk_pos > 0:
                    ...
                    # Extract amount from the specific position in the line
                    # The amount is a 7-digit number that appears after the asterisk section
                    if len(line) > asterisk_pos + 30:  # Ensure we have enough characters
                        # Get the 7 digits that represent the amount
                        amount_str = line[asterisk_pos + 23:asterisk_pos + 30]
                        logger.debug(f'Amount string from specific position: {amount_str}')
                    ...
                    if date_match:
                        ...
                        logger.debug(f'Added date {formatted_date} to ATM ref {atm_ref_field}')
